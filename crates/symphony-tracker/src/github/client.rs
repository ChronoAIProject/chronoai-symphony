...
impl GitHubClient {
    ...
    async fn fetch_paginated(&self, state_param: &str) -> Result<Vec<Value>, SymphonyError> {
    ...
    async fn fetch_issue_by_number(&self, number: u64) -> Result<Value, SymphonyError> {
        let token = self.get_token().await?;
        let url = format!(
            "{}/repos/{}/{}/issues/{}",
    ...
    fn normalize_items(&self, items: &[Value]) -> Vec<Issue> {
        items
            .iter()
            .filter(|item| !item.get("pull_request").is_some_and(|v| !v.is_null()))
            .filter_map(|item| {
                normalize_github_issue(item, &self.active_states, &self.terminal_states)
            })
            .collect()
    }
}
...
impl IssueTracker for GitHubClient {
    ...
    async fn fetch_issues_by_states(&self, states: &[String]) -> Result<Vec<Issue>, SymphonyError> {
        ...
        let has_active = states_lower
            .iter()
            .any(|s| self.active_states.iter().any(|a| a.to_lowercase() == *s) || *s == "todo");
        ...
        info!(count = filtered.len(), ?states, "fetched issues by states");
        Ok(filtered)
    }
        ...
        for id in ids {
            match extract_issue_number(id) {
                Some(n) => match self.fetch_issue_by_number(n).await {
                    Ok(raw) => {
                        if let Some(issue) =
                            normalize_github_issue(&raw, &self.active_states, &self.terminal_states)
                        {
                            results.push(issue);
                        }
                    }
...
fn parse_project_slug(slug: &str) -> Result<(String, String), SymphonyError> {
